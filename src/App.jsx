...
const EMAILJS_TEMPLATE_RAPPEL = "template_3w471uo"; // réutilise le template protocole avec message adapté
...
function PolitiqueConfidentialite({ onClose }) {
  return (
    <div style={{ minHeight: "100vh", background: bg, padding: "40px 20px", fontFamily: "sans-serif" }}>
      <div style={{ maxWidth: 700, margin: "0 auto" }}>
        <button onClick={onClose} style={{ ...btn("ghost"), fontSize: 12, padding: "6px 14px", marginBottom: 28 }}>← Retour</button>
        <h1 style={{ fontFamily: "serif", fontSize: 28, color: tx, fontWeight: 700, marginBottom: 8 }}>Politique de confidentialité</h1>
        <p style={{ color: td, fontSize: 13, marginBottom: 32 }}>Dernière mise à jour : avril 2026</p>

        {[
          ["1. Responsable du traitement", `Meije — Naturopathe fonctionnelle
Contact : lamenaturo@gmail.com
Site : meije-suivi.vercel.app`],
          ["2. Données collectées", `Dans le cadre de l'accompagnement naturopathique, nous collectons les données suivantes :

• Données d'identification : prénom, adresse email
• Données de santé : questionnaire de santé complet (antécédents médicaux, symptômes, historique hormonal, habitudes de vie, bilans biologiques)
• Données de suivi hebdomadaire : symptômes, humeur, prise de compléments, cycle menstruel
• Documents uploadés : bilans sanguins, ordonnances (photos et PDF)
• Messages échangés avec la praticienne`],
          ["3. Finalité du traitement", `Ces données sont collectées exclusivement pour :

• Préparer et personnaliser les consultations naturopathiques
• Suivre l'évolution de votre état de santé au cours de l'accompagnement
• Vous envoyer votre protocole personnalisé
• Vous envoyer des rappels de suivi hebdomadaire`],
          ["4. Base légale", `Le traitement de vos données repose sur :
• Votre consentement explicite (création de compte et remplissage du questionnaire)
• L'exécution du contrat d'accompagnement naturopathique

Conformément à l'article 9 du RGPD, le traitement de données de santé est autorisé dans le cadre d'un suivi individuel par un professionnel de santé ou de bien-être, avec votre consentement explicite.`],
          ["5. Hébergement et sous-traitants", `Vos données sont hébergées par :

• Firebase (Google LLC) — base de données et authentification — serveurs en Europe (Frankfurt)
• Cloudinary — stockage des fichiers uploadés (bilans, documents)
• Vercel — hébergement du site web
• EmailJS — envoi des emails de notification

Ces prestataires sont soumis à des garanties de protection des données conformes au RGPD.`],
          ["6. Durée de conservation", `Vos données sont conservées pendant la durée de votre accompagnement et jusqu'à 3 ans après la fin de celui-ci, conformément aux recommandations pour les professionnels de santé.

Vous pouvez demander la suppression de vos données à tout moment.`],
          ["7. Vos droits", `Conformément au RGPD, vous disposez des droits suivants :
...
Pour exercer ces droits, contactez : lamenaturo@gmail.com

En cas de litige, vous pouvez saisir la CNIL : www.cnil.fr`],
          ["8. Sécurité", `Vos données sont protégées par :
• Authentification sécurisée (email + mot de passe chiffré)
• Règles d'accès Firestore : chaque utilisatrice ne peut accéder qu'à ses propres données
• Transmission chiffrée (HTTPS)
• Accès limité à la praticienne pour les données de suivi`],
          ["9. Cookies", `Ce site n'utilise pas de cookies publicitaires ou de tracking. Des cookies techniques essentiels au fonctionnement de l'authentification peuvent être utilisés.`],
        ].map(([titre, texte]) => (
          <div key={titre} style={{ marginBottom: 28 }}>
            <h2 style={{ color: ac, fontSize: 16, fontWeight: 700, marginBottom: 10 }}>{titre}</h2>
            <p style={{ color: tm, fontSize: 14, lineHeight: 1.8, whiteSpace: "pre-line" }}>{texte}</p>
          </div>
        ))}

        <div style={{ background: sf, borderRadius: 12, border: "1px solid " + bd, padding: 20, marginTop: 32 }}>
          <p style={{ color: td, fontSize: 13, lineHeight: 1.7 }}>
            Pour toute question relative à vos données personnelles, contactez Meije à l'adresse : <span style={{ color: ac }}>lamenaturo@gmail.com</span>
          </p>
        </div>
      </div>
    </div>
...
function Auth({ onLogin }) {
  ...
  if (showPrivacy) return <PolitiqueConfidentialite onClose={() => setShowPrivacy(false)} />;
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, background: bg }}>
      ...
      <p style={{ color: td, fontSize: 11, textAlign: "center", marginTop: 16 }}>
        Suivi confidentiel · <button onClick={() => setShowPrivacy(true)} style={{ background: "none", border: "none", color: td, fontSize: 11, cursor: "pointer", textDecoration: "underline", fontFamily: "sans-serif" }}>Politique de confidentialité</button>
      </p>
    </div>
  );
}
                ...
                {complements.map((c, i) => {
                  ...
                  <div key={i} style={{ marginBottom: 10 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                      <span style={{ color: tm, fontSize: 13 }}>{nom}</span>
                      {lien && <a href={lien} target="_blank" rel="noreferrer" style={{ color: ac, fontSize: 12, textDecoration: "underline" }}>→ Commander</a>}
                    </div>
...
function Praticienne({ user, onLogout }) {
  ...
  const addComplement = async () => {
    ...
    const item = newComplement.lien.trim()
      ? { nom: newComplement.nom.trim(), lien: newComplement.lien.trim() }
      : { nom: newComplement.nom.trim(), lien: "" };
  ...
  const toggleRappels = async () => {
    const current = clientData && clientData.rappelsActifs !== false;
    await updateDoc(doc(db, "users", selected.uid), { rappelsActifs: !current });
  };
  ...
  const sendProtocole = async () => {
    ...
    try {
      await sendEmail(EMAILJS_TEMPLATE, {
        prenom: selected.prenom,
        email: selected.email,
        to_email: selected.email,
        titre: newProtocole.titre.trim(),
        message: "Meije vient de mettre ton protocole à disposition sur ton espace suivi.",
      });
          ...
          <div>
            ...
            {activeTab === "complements" && (
              <div>
                  ...
                  ? clientData.complements.map((c, i) => {
                    ...
                    return (
                      <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: sf, borderRadius: 10, padding: "12px 16px", marginBottom: 8 }}>
                        <div>
                          <span style={{ color: tx, fontSize: 14 }}>{nom}</span>
                          {lien && <a href={lien} target="_blank" rel="noreferrer" style={{ display: "inline-block", marginLeft: 10, color: ac, fontSize: 12, textDecoration: "underline" }}>→ Commander</a>}
                        </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 20, marginBottom: 8, background: sf, borderRadius: 10, padding: "12px 16px" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ color: tx, fontSize: 14, fontWeight: 600 }}>Rappels automatiques</div>
                    <div style={{ color: td, fontSize: 12 }}>Email si pas de suivi depuis 8 jours</div>
                  </div>
                  <button onClick={toggleRappels} style={{ padding: "6px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontFamily: "sans-serif", fontSize: 12, fontWeight: 600, background: clientData?.rappelsActifs !== false ? ac : "rgba(255,255,255,0.08)", color: clientData?.rappelsActifs !== false ? "#0c0f0e" : tm }}>
                    {clientData?.rappelsActifs !== false ? "Actifs" : "Désactivés"}
                  </button>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 16 }}>
                  <input value={newComplement.nom} onChange={e => setNewComplement(f => ({ ...f, nom: e.target.value }))} placeholder="Nom du complément — Ex : Magnésium 300mg/j au dîner" style={iS} />
                  <input value={newComplement.lien} onChange={e => setNewComplement(f => ({ ...f, lien: e.target.value }))} placeholder="Lien (optionnel) — Ex : https://www.sunday-natural.fr/..." style={iS} />
                  <button onClick={addComplement} disabled={savingComplements} style={{ ...btn("primary"), alignSelf: "flex-start" }}>Ajouter</button>
                </div>
              </div>
            )}
