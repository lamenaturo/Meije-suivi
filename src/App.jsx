...
// ── États édition nom cliente ──
const [editingClientNom,setEditingClientNom]=useState(false);
const [editClientFields,setEditClientFields]=useState({prenom:””,nom:””});
const [savingClientNom,setSavingClientNom]=useState(false);
...
const saveClientNom=async()=>{if(!selected)return;setSavingClientNom(true);const prenom=editClientFields.prenom.trim();const nom=editClientFields.nom.trim();if(!prenom){setSavingClientNom(false);return;}await updateDoc(doc(db,“users”,selected.uid),{prenom,…(nom?{nom}:{})});setSelected(prev=>({…prev,prenom,…(nom?{nom}:{})}));setEditingClientNom(false);setSavingClientNom(false);showToast(“Nom mis à jour ✓”);};
      ...
      <div style={{background:P.pSurface,borderRadius:14,border:`1px solid ${P.pBorder}`,padding:"18px 20px",marginBottom:18,display:"flex",alignItems:"center",gap:16}}>
        ...
        <div style={{flex:1}}>
          {editingClientNom?(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <input value={editClientFields.prenom} onChange={e=>setEditClientFields(f=>({...f,prenom:e.target.value}))} placeholder="Prénom" style={{...iP("p"),fontSize:14,flex:1,padding:"8px 12px"}}/>
                <input value={editClientFields.nom} onChange={e=>setEditClientFields(f=>({...f,nom:e.target.value}))} placeholder="Nom (optionnel)" style={{...iP("p"),fontSize:14,flex:1,padding:"8px 12px"}}/>
              </div>
              <div style={{display:"flex",gap:8}}>
                <Btn onClick={saveClientNom} disabled={savingClientNom||!editClientFields.prenom.trim()} variant="primary" small>{savingClientNom?"Enregistrement…":"Enregistrer"}</Btn>
                <Btn onClick={()=>setEditingClientNom(false)} variant="ghost" theme="p" small>Annuler</Btn>
              </div>
            </div>
          ):(
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <p style={{fontFamily:P.serif,fontSize:20,color:P.pText,fontWeight:400}}>{selected.prenom}{selected.nom?" "+selected.nom:""}</p>
              <button onClick={()=>{setEditClientFields({prenom:selected.prenom||"",nom:selected.nom||""});setEditingClientNom(true);}} style={{background:"none",border:"none",color:P.pTextDim,fontSize:14,cursor:"pointer",padding:"2px 4px",lineHeight:1}} title="Modifier le nom">✏️</button>
            </div>
          )}
